...
class GraphWidget(HasTraits):
    ...
    def _observe_points(self, change):
        ...
        faces = stable_delaunay_faces(self.points, alpha_threshold=self.alpha)
        edges = faces_to_edges(faces)
        self.edges = self.points[edges].tolist()
...
class TEMNetMetadataLoader(HasTraits):
    ...
    def _observe_frame_data(self, change):
        ...
        if first_defect_frame is not None:
            ...
            faces = stable_delaunay_faces(points, alpha_threshold=2)
            adjacency = faces_to_adjacency(faces, len(points))
            # graph = stable_delaunay_graph(points, 2)
            adjacent_labels = [labels[j - 1] for j in adjacency[0]]
